...
func (h GIFHandler) Status(w http.ResponseWriter, r *http.Request) {
	status := h.service.Renderer.Status()
	writeJSON(w, http.StatusOK, map[string]any{
		"ok":               true,
		"gifEnabled":       h.cfg.GIFEnabled,
		"ffmpegAvailable":  status.Available,
		"ffmpegSafe":       status.Safe,
		"ffmpegBin":        status.Bin,
		"ffmpegVersion":    status.Version,
		"ffmpegMinVersion": status.MinimumVersion,
		"ffmpegStatusCode": status.Code,
		"ffmpegMessage":    status.Message,
...
func (h GIFHandler) resolveFramePaths(job jobs.Job, requested []int, maxFrames int) ([]string, []int, error) {
	...
	for _, index := range requested {
		...
		if result.Mime != "" && output.NormalizeMime(result.Mime) != "image/png" {
			return nil, nil, fmt.Errorf("帧 %d 不是 PNG 输出，不能合成 GIF", index)
		}
		path, mime, err := h.output.Resolve(job.SpaceToken, result.OutputDate, result.OutputFileName)
		if err != nil {
			return nil, nil, err
		}
		if output.NormalizeMime(mime) != "image/png" {
			return nil, nil, fmt.Errorf("帧 %d 不是 PNG 输出，不能合成 GIF", index)
		}
...
func gifAPIErrorCode(err error) string {
	...
	case strings.Contains(text, "PNG"):
		return "GIF_FRAME_FORMAT_UNSUPPORTED"
