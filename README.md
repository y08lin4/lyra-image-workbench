...
- Docker 镜像：`ghcr.io/y08lin4/lyra-image-workbench:latest`
...
| 模型 | `image-2`、`image-2（满血版）`，兼容旧 `gpt-image-2` 输入；具体上游模型由渠道配置决定 |
...
| 开源部署 | Go 单进程托管 API + 静态前端；支持本机、Docker/GHCR、Linux systemd、宝塔/Nginx 反代 |

> Banana Nano / Banana provider 已迁移到独立 `banana` 分支；当前 `dev` 文档和公开功能面不再把 Banana 作为可用功能。
...
- `codex-key` 默认保存在当前浏览器 `localStorage`。
