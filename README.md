...
一个面向本机 / 私有服务器部署的 AI 生图工作台。项目由 Go 后端统一托管前端页面和同源 `/api`，前端不直连 NewAPI，也不保存上游地址；图片生成、任务队列、Key 使用、结果落盘、SSE 状态推送都由后端负责。
...
结果卡片里可能出现“上游改写提示词”。这不是本项目主动改写，也不是提示词助手生成。
...
- 使用单独 Banana 分组 Key
- URL 仍走 Admin 配置的 NewAPI Base URL
