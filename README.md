...
一个面向本机 / 私有服务器部署的 LyAI 生图工作台。项目由 Go 后端统一托管前端页面和同源 `/api`，前端不直连 NewAPI，也不保存上游地址；API Key 只保存在当前浏览器本地，请求时临时交给 Go 后端代理使用；图片生成、任务队列、结果落盘、SSE 状态推送都由后端负责。
...
- `codex-key`：保存在当前浏览器本地，Image-2 与提示词助手请求时临时使用
- Banana 分组 Key：保存在当前浏览器本地，Banana Nano 请求时临时使用
- 默认数量：按当前账号保存到后端
- 默认并发：按当前账号保存到后端
- PiXhost 自动上传开关：按当前账号保存到后端

Key 不写入服务器 `data/spaces/{storageToken}/config.json`，`GET /api/config` 也不会返回明文 Key。更换浏览器、清空浏览器数据或换设备登录后，需要在当前浏览器重新填写 Key；历史任务、参考图、提示词历史和默认生成设置仍保存在服务器账号空间里。
...
- 浏览器按当前登录用户名在 `localStorage` 保存 `codex-key` 与 Banana 分组 Key。
- 创建任务、重试任务、提示词助手调用时，前端通过临时请求头把对应 Key 传给同源后端。
- 后端只在本次请求和正在运行的任务内存中使用 Key，不持久化 Key。
- 服务器重启后，运行中任务会标记为 `interrupted`；重试时需要当前浏览器再次提供本地 Key。
...
2. 后端创建任务并立刻返回任务对象。
3. 后端把本次请求带来的浏览器本地 Key 暂存到内存，只供该任务调用上游使用。
...
- 已完成任务保留。
- 排队任务可恢复。
- 运行中任务会标记为 `interrupted`，避免重复请求导致重复扣费；由于 Key 只在浏览器本地保存，重试时需要当前浏览器重新提供 Key。
...
```bash
git config --global --add safe.directory /www/wwwroot/image-workbench && cd /www/wwwroot/image-workbench && git pull --ff-only && cd web && npm ci && npm run build && cd .. && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags="-s -w" -o image-workbench-local-server ./cmd/local-server && chmod +x image-workbench-local-server && chown -R www:www /www/wwwroot/image-workbench && systemctl restart image-workbench && systemctl status image-workbench --no-pager
```
