...
The published Docker image installs `ffmpeg` in the Alpine runtime stage, so `/gif` final rendering works inside the container by default. FFmpeg is still treated as an external command dependency: it is not bundled into the Go binary and cgo is not used.
...
If `ffmpeg -version` is not available inside the container, `/gif` can still generate frames but the merge button is disabled.
