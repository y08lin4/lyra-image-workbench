...
> 历史说明：本文是第一阶段开发过程中的任务调度档案，保留当时的分支合并和子代理分工记录；当前现役功能范围以 README、docs/ROUTES.md 和实际路由为准。视频/MiniMax、GIF 工作台和 FFmpeg 渲染链路已从当前功能面移除。
...
8. 当前功能面移除视频/MiniMax、GIF 工作台和 FFmpeg 渲染链路；历史中提到的 GIF/FFmpeg 仅作为当时分支记录，不作为当前目标。
