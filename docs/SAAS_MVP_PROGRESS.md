...
- 后端删除目标、禁止误删 GIF/FFmpeg 的边界、残留扫描命令已在 `docs/AGENT_TASK_BREAKDOWN.md` 写清。
- 当前工作区已有后端 MiniMax 文件删除和 README/测试/设置相关改动痕迹，说明 A4-impl 已进入实现阶段。

未完成边界：前端仍存在 `MiniMaxVideoPanel`、`minimaxVideos.ts`、视频 tab、管理页 MiniMax Key/视频额度 UI，因此不能宣称视频功能已完全移除。
...
2. A4-impl 后端视频删除：后端 MiniMax 文件和文档已有删除痕迹，GIF/FFmpeg 文件仍保留；还需要完成残留扫描、测试和前端视频删除协同。
...
6. 前端视频删除：删除视频一级 tab、MiniMax 管理配置、视频额度 UI、`MiniMaxVideoPanel.tsx`、`minimaxVideos.ts` 和相关类型/API。
7. 全量测试：`go test ./...`、前端 `npm run build`、残留扫描、安全扫描。
8. 启动验收：本地启动后完成注册、充值/回调、广场投稿、点赞榜单、资料页、API 文档、管理页、主题切换、GIF 保留检查。
...
5. GIF/FFmpeg 安全风险：删除视频时必须保留 GIF；FFmpeg 只处理项目生成的 PNG 帧，不接受用户上传视频进入解码链路；部署侧继续要求已修复高危漏洞的 FFmpeg 版本，当前文档中以 `8.1.2+` 为安全门槛。
...
11. 确认 MiniMax/视频入口已移除，GIF 工作台和 FFmpeg 安全降级仍可用。
...
5. 前端视频/MiniMax 与后端 MiniMax 已删除；GIF/FFmpeg 保留，README 已加入 FFmpeg 8.1.2+ 安全提醒。
