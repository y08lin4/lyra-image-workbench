...
- 前端不把 codex-key 带去 NewAPI。
- Go 后端是唯一请求 NewAPI 的模块。
- NewAPI 地址内置在 Go 后端配置里，用户只填 codex-key。
...
| codex-key 保存 | 浏览器保存 Key 不符合目标 | codex-key 只通过 `POST /api/config` 交给 Go；Go 本机保存，`GET /api/config` 只返回 `apiKeySet` 和掩码，同时返回默认并发 |
...
- 不让前端拿远程图片 URL 直接展示。
- 不把图床上传作为任务成功条件。
- 不在日志输出 codex-key、完整 Authorization、完整请求体。
...
1. 配置模块：内置 NewAPI URL、codex-key 保存、`GET/POST /api/config`。
