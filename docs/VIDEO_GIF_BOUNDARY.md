...
> 历史快照提醒：本文记录的是 2026-06-27 当时的视频/GIF 边界扫描结果，其中“没有独立 GIF 动态模块”的结论已经过期。当前 `dev` 分支已上线独立 GIF 页面和 `mode=gif` 本地动图任务，现役实现以 `docs/GIF_IMPLEMENTATION.md` 为准；视频/MiniMax/FFmpeg 仍不属于当前 GIF 实现。
